...
#endif
{
		...
		//Reading data from LFSR VALUE ADDRESS (LFSR_VAL_BASE)
	volatile int data = IORD_ALTERA_AVALON_PIO_DATA(LFSR_VAL_BASE);
	//checking if modulo 2 of data is 0 or 1 and writing the corresponding value in DDS_INCREMENT
	if(data % 2 == 1)
		IOWR_ALTERA_AVALON_PIO_DATA(DDS_INCREMENT_BASE,430); //  5Hz
	else
		IOWR_ALTERA_AVALON_PIO_DATA(DDS_INCREMENT_BASE,86); //  1Hz
	//reset edge capture mechanism 
